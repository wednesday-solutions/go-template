...
func (r *mutationResolver) Login(
	...
	if err != nil {
		return nil, fmt.Errorf(
			"error in creating auth service ",
		)
	}

	if !u.Password.Valid ||
		(!sec.HashMatchesPassword(u.Password.String, password)) {
		return nil, fmt.Errorf(
	...
	refreshToken := sec.Token(
		token,
	)
	u.Token = null.StringFrom(
		refreshToken,
	)
...
func (r *mutationResolver) ChangePassword(
	...
	if !sec.HashMatchesPassword(
		convert.NullDotStringToString(
			u.Password,
		),
		oldPassword,
	) {
	...
	return &gqlmodels.ChangePasswordResponse{
		Ok: true,
	}, err
}
...
func (r *mutationResolver) RefreshToken(
	...
	resp, err := tg.GenerateToken(
		user,
	)
	if err != nil {
		return nil, err
	}
	return &gqlmodels.RefreshTokenResponse{
		Token: resp,
	}, nil
}

// Mutation returns
// gqlmodels.MutationResolver
// implementation.
