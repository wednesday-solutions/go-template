...
func (r *mutationResolver) CreateUser(
	ctx context.Context,
	input gqlmodels.UserCreateInput,
	...
	roleId, _ := strconv.Atoi(
		input.RoleID,
	)
	user := models.User{
		Username: null.StringFrom(
			input.Username,
		),
	...
	user.Password = null.StringFrom(
		sec.Hash(
			user.Password.String,
		),
	)
	newUser, err := daos.CreateUserTx(
		user,
		nil,
	)
	if err != nil {
	...
	graphUser := convert.UserToGraphQlUser(
		&newUser,
		1,
	)
...
func (r *mutationResolver) UpdateUser(
	ctx context.Context,
	input *gqlmodels.UserUpdateInput,
) (*gqlmodels.User, error) {
	userID := auth.UserIDFromContext(
		ctx,
	)
	user, _ := daos.FindUserByID(
		userID,
	)
	var u models.User
	if user != nil {
	...
	if input.FirstName != nil {
		u.FirstName = null.StringFromPtr(
			input.FirstName,
		)
	}
	if input.LastName != nil {
		u.LastName = null.StringFromPtr(
			input.LastName,
		)
	}
	if input.Mobile != nil {
		u.Mobile = null.StringFromPtr(
			input.Mobile,
		)
	}
	if input.Address != nil {
		u.Address = null.StringFromPtr(
			input.Address,
		)
	}
	_, err := daos.UpdateUserTx(
		u,
		nil,
	)
	if err != nil {
...
func (r *mutationResolver) DeleteUser(
	ctx context.Context,
) (*gqlmodels.UserDeletePayload, error) {
	userID := auth.UserIDFromContext(
		ctx,
	)
	u, err := daos.FindUserByID(
		userID,
	)
	if err != nil {
		return nil, resultwrapper.ResolverSQLError(
	...
	return &gqlmodels.UserDeletePayload{
		ID: fmt.Sprint(
			userID,
		),
	}, nil
