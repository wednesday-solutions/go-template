		...
		t.Run(
			...
			func(t *testing.T) {
				...
				if tt.wantErr {
					...
					mock.ExpectQuery(regexp.QuoteMeta("SELECT `users`.* FROM `users` WHERE (username=?) LIMIT 1;")).
						WithArgs().
	...
	for _, tt := range cases {
		t.Run(
			...
			func(t *testing.T) {
				...
				if tt.wantErr {
					// get user by token
					mock.ExpectQuery(regexp.QuoteMeta("SELECT `users`.* FROM `users` WHERE (token=?) LIMIT 1;")).
						WithArgs().
						WillReturnError(fmt.Errorf(""))
				}
				// get user by token
