...
import (
	...
	"github.com/wednesday-solutions/go-boiler/gqlgen/boiler"
)
...
func (m *Plugin) MutateConfig(originalCfg *config.Config) error {
	...
	fmt.Println("[convert] render preload.gotpl", m.directory)
...
func getShortType(longType string) string {
	//*github.com/wednesday-solutions/go-boiler/gqlgen/examples/social-network/graphql_models.StringFilter
...
func getConvertConfig(model *Model, field *Field) (cc ConvertConfig) {
	...
	if graphType != boilType {
		...
		if field.IsPrimaryID || field.IsID {
			...
			goToInt := getBoilerTypeAsText(boilType) + "ToInt"
			if goToInt != "IntToInt" {
				cc.ToGraphQL = "helper." + goToInt + "(VALUE)"
			}
