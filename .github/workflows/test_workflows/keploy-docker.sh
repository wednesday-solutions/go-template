...
sudo docker-compose --env-file ./.env.docker -f docker-compose.yml build
...
sudo -E env PATH="$PATH" /usr/local/bin/keployE test -c "sudo docker-compose --env-file ./.env.docker -f docker-compose.yml up" --containerName "got" --delay 40 --apiTimeout 300 --generateGithubActions=false
